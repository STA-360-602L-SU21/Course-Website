...
1. Update $U$: first, sample $u^\star \sim g_u[u^\star | u^{(s)}, v^{(s)}]$. Then,
    ...
    + Sample $w \sim U(0,1)$. Set $u^{(s+1)}$ to $u^\star$ if $w < r$, or set $u^{(s+1)}$ to $u^{(s)}$ otherwise.
...
2. Update $V$: first sample $v^\star \sim g_v[v^\star | u^{(s+1)}, v^{(s)}]$. Then,
  ...
  + Sample $w \sim U(0,1)$. Set $v^{(s+1)}$ to $v^\star$ if $w < r$, or set $v^{(s+1)}$ to $v^{(s)}$ otherwise.
